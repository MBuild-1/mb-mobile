...
class CropState extends State<Crop> with TickerProviderStateMixin, Drag {
  ...
  bool _hasSetToMinimumScale = false;
  ...
  void _updateImage(ImageInfo imageInfo, bool synchronousCall) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      setState(() {
        ...
        if (!_hasSetToMinimumScale) {
          _hasSetToMinimumScale = true;
          _scaleToMinimum(imageInfo);
        }
      });
    });
  ...
  void _scaleToMinimum(ImageInfo imageInfo) async {
    final image = imageInfo.image;
    final boundaries = _boundaries;
    if (boundaries == null) {
      return;
    ...
    setState(() {
      ...
      final areaWidth = boundaries.width * _area.width;
      final areaHeight = boundaries.height * _area.height;
      final imageWidth = image.width * _scale * _ratio;
      final imageHeight = image.height * _scale * _ratio;
      final dx = ((imageWidth - areaWidth) / 2.0).abs();
      final dy = ((imageHeight - areaHeight) / 2.0).abs();
      _view = Rect.fromLTWH(
        (_area.left * _view.width / _scale) - (dx / imageWidth),
        (_area.top * _view.height / _scale) - (dy / imageHeight),
        _view.width,
        _view.height
      );
      _view = _getViewInBoundaries(_scale);
